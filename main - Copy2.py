...
def create_account():
    ...
    create_ac = Button(sign_up_frame, image=sign_up_btn, command=insert_info, bd=0)
    back_btn = Button(sign_up_frame, image=back_button2, command=back, bd=0)
...
def login_page():
    ...
    log_ac = Button(login_frame, image=login_btn, command=login_acc, bd=0)
    back_btn = Button(login_frame, image=back_button2, command=back,  bd=0)
...
def home_page():
    ...
    def order_page():
        ...
        def display_order():
            ...
            for x in results_my_cursor_2:  # Displaying The Table
                for j in range(len(x)):
                    e = Label(order_frame_2, width=22, text=x[j], font=('roboto', 9), anchor="w", bg='#ffffff')
                    e.grid(row=i, column=j, padx=5, pady=10)
